...
module.exports = {
  ...
  createLead: (leadData, callback) => {
    pool.query(
      `INSERT INTO lead_status_master 
            ...
            VALUES (?, ?, ?)`,
      [leadData.status_name, leadData.display_order, leadData.is_active],
      (err, result) => {
        if (err) {
  ...
  getAllLeads: (callback) => {
    pool.query(
      `SELECT status_id, status_name, display_order,is_followup_date_required, is_active ,requires_followup,is_call_required,is_policy_required
